...
const mediaAnnotation = [
    {
        ...
        newsletter: {
            // Lia@Note 03-02-26 --- You can change jid, message id, and name via .env (â â‰§â â–½â â‰¦â )
...
const prepareStickerPackMessage = async (message, options) => {
    const { cover, stickers = [], name = 'ðŸ“¦ Sticker Pack', publisher = 'GitHub: nozez-lab', description = 'ðŸ·ï¸ nozez-lab/baileys' } = message;
        ...
        for (let j = i; j < chunkEnd; j++) {
            promises.push((async (index) => {
                ...
                stickerMetadata[index] = {
                    ...
                    accessibilityLabel: sticker.accessibilityLabel || 'â€Ž'
                };
            })(j));
        }
    ...
    if (hasOptionalProperty(message, 'optionText') && !!message.optionText) {
        Object.assign(messageParamsJson, {
            bottom_sheet: {
                ...
                list_title: message.optionTitle || 'ðŸ“„ Select Options',
            ...
            if (hasOptionalProperty(button, 'id') && !!button.id) {
                return {
                    ...
                    buttonParamsJson: JSON.stringify({
                        display_text: buttonText || 'ðŸ‘‰ðŸ» Click',
                        id: button.id,
                        icon: buttonIcon
                    })
                };
                ...
                return {
                    name: 'cta_copy',
                    buttonParamsJson: JSON.stringify({
                        display_text: buttonText || 'ðŸ“‹ Copy',
                ...
                return {
                    name: 'cta_url',
                    buttonParamsJson: JSON.stringify({
                        display_text: buttonText || 'ðŸŒ Visit',
                        url: button.url,
                ...
                return {
                    name: 'cta_call',
                    buttonParamsJson: JSON.stringify({
                        display_text: buttonText || 'ðŸ“ž Call',
                        phone_number: button.call,
                        icon: buttonIcon
                    })
                };
            }
            // Lia@Changes 12-03-26 --- Add "single_select" shortcut ï¼¼â (â Â°â oâ Â°â )â ï¼
            else if (hasOptionalProperty(button, 'sections') && !!button.sections) {
                return {
                    name: 'single_select',
                    buttonParamsJson: JSON.stringify({
                        title: buttonText || 'ðŸ“‹ Select',
...
// Lia@Changes 06-02-26 --- Validate album message media to avoid bug ðŸ‘€
...
// Lia@Changes 30-01-26 --- Validate carousel cards header to avoid bug ðŸ‘€
...
export const generateWAMessageContent = async (message, options) => {
    ...
    // Lia@Note 03-02-26 --- This message type is shown on WhatsApp Web/Desktop and iOS (I guess ï½¡â â—•â â€¿â â—•â ï½¡). On Android, it only appears in newsletter (so far à²¥â â€¿â à²¥)
            ...
            hydratedButtons: message.templateButtons.map((button, i) => {
                ...
                if (hasOptionalProperty(button, 'id')) {
                    return {
                        index: i,
                        quickReplyButton: {
                            displayText: buttonText || 'ðŸ‘‰ðŸ» Click',
                            id: button.id
                        }
                    };
                }
                    ...
                    return {
                        index: i,
                        urlButton: {
                            displayText: buttonText || 'ðŸŒ Visit',
                            url: button.url
                        }
                    };
                }
                    ...
                    return {
                        index: i,
                        callButton: {
                            displayText: buttonText || 'ðŸ“ž Call',
        ...
        hydratedTemplate.templateId = message.id || 'template-' + Date.now(); // Lia@Note 04-02-26 --- Minimal templateId to satisfy WhatsApp (â Â â êˆâ á´—â êˆâ )
        m = {
            templateMessage: {
    ...
    // Lia@Changes 02-02-26 --- Add "interactiveAsTemplate" boolean to wrap interactiveMessage into templateMessage
        ...
        if (!m.interactiveMessage) {
            throw new Boom('Invalid message type for template', { statusCode: 400 }); // Lia@Note 02-02-26 --- To avoid bug ðŸ‘€
        }
        m = {
            templateMessage: {
                interactiveMessageTemplate: m.interactiveMessage,
                templateId: message.id || 'template-' + Date.now() // Lia@Note 04-02-26 --- Minimal templateId to satisfy WhatsApp (â Â â êˆâ á´—â êˆâ )
            }
        };
